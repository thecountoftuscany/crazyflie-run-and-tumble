...
void radiolinkSyslinkDispatch(SyslinkPacket *slp)
{
  ...
  } else if (slp->type == SYSLINK_ALT_RSSI)  // custom for another bluetooth source
  {
    //Extract RSSI sample sent from custom radio
    memcpy(&rssi_alt, slp->data, sizeof(uint8_t)); //rssi will not change on disconnect
...
LOG_ADD(LOG_UINT8, rssi_alt, &rssi_alt)  // custom for another bluetooth device
